          ...
          >
            <div className="p-6 border-b border-kaia-cream flex justify-between items-center bg-kaia-cream/30">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-3xl font-display text-kaia-charcoal">
                  Your Pantry Bag ({totalItems})
                </h2>
              </div>
              <button
                onClick={onClose}
                className="p-2 hover:bg-kaia-cream rounded-full transition-colors"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="grow overflow-y-auto p-6 space-y-6">
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <p className="text-kaia-taupe font-script text-3xl">
                    Pantry is empty
                  </p>
                  <button
                    onClick={onClose}
                ...
                cart.map((item) => (
                  ...
                  >
                    <div className="w-24 h-24 bg-kaia-cream rounded-2xl shrink-0 p-2 border border-kaia-tan/30">
                      <img
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="grow">
                      <div className="flex justify-between mb-1">
                        <div>
                          <h4 className="text-xl font-display text-kaia-charcoal">
                            {item.name}
                          </h4>
                          {item.slices && (
                      ...
                      <p className="text-kaia-red font-display text-lg mb-3">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center border border-kaia-tan rounded-xl overflow-hidden">
                          <button
                            onClick={() =>
                              updateQuantity(item.id, -1, item.slices)
                            }
            ...
            {cart.length > 0 && (
              <div className="p-8 border-t border-kaia-tan/30 bg-kaia-cream/20 space-y-6">
                <div className="flex justify-between items-center">
                  <span className="text-kaia-taupe font-bold uppercase tracking-widest text-xs">
                    Subtotal
                  </span>
                  <span className="text-4xl font-display text-kaia-red">
